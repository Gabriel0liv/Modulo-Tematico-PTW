                    ...
                    @forelse($anuncios as $anuncio)
                        @if($anuncio->ativo == 1)
                            <div class="bg-white border rounded-lg shadow-sm overflow-hidden relative">
                                <form action="{{ route('anuncio.desativar', ['tipo' => $anuncio->tipo_produto, 'id' => $anuncio->id]) }}" method="POST" class="absolute top-2 right-2 z-10">
                                    @csrf
                                    <button type="submit" class="p-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-lg transition duration-200 opacity-90 hover:opacity-100">
