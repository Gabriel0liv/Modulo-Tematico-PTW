    ...
    <main class="max-w-6xl mx-auto p-6">
        <div class="flex items-start justify-between mb-8">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">Seu Carrinho
                @if(!empty($item))
                    <span class="text-primary-light">(1 item)</span>
                @else
                    ...
                    <div class="bg-white rounded-xl shadow-soft p-5 hover:shadow-medium transition-all duration-300">
                        <div class="flex flex-col sm:flex-row">
                            <div class="w-full sm:w-24 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center mb-4 sm:mb-0 sm:mr-5">
                                <img src="{{ $item['imagem'] ?? '/img/destaque.png' }}" alt="{{ $item['nome'] }}" class="h-full w-full object-contain">
                            </div>
                            <div class="flex-1">
                                <div class="flex flex-col sm:flex-row justify-between">
                                    <div>
                                        <h3 class="font-semibold text-gray-800 text-lg">{{ $item['nome'] }}</h3>
                                        <p class="text-sm text-gray-500 mb-1">{{ ucfirst($item['tipo'] ?? 'destaque') }}</p>
                                    </div>
                    ...
                    @if($item)
                        <div class="space-y-3 mb-6">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-medium">€{{ number_format($item['preco'], 2, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                        ...
                        <div class="flex justify-between mb-6">
                            ...
                            <span class="font-bold text-lg text-gray-800">€{{ number_format($item['preco'], 2, ',', '.') }}</span>
                        </div>
                        <a href="{{ route('checkout.destaque') }}" class="block w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 text-center rounded-lg shadow-soft transition-all">
                            Finalizar Pedido
                        </a>
                    @endif
                </div>
